    ...
    if(!strncmp(ELFMAG, header_elf->e_ident, SELFMAG)) {
        if(header_elf->e_type == ET_DYN) {
            for(int sect_idx = 0; sect_idx < header_elf->e_shnum; sect_idx++) {
                header_sect = (buffer + header_elf->e_shoff) + (sect_idx * header_elf->e_shentsize);
                if(header_sect->sh_type == SHT_SYMTAB) {
                    header_link = (buffer + header_elf->e_shoff) + (header_sect->sh_link * header_elf->e_shentsize);
                    for(int sym_idx = 0; sym_idx < header_sect->sh_size / header_sect->sh_entsize; sym_idx++) {
                        symbol = (buffer + header_sect->sh_offset) + (sym_idx * header_sect->sh_entsize);
...
// TODO: Make this compatible with Windows
int inject(injector_pid_t victim, char *script_path) {
    uint8_t *buffer;
    int error_code;
    injector_t *injector;
    FILE *original;
